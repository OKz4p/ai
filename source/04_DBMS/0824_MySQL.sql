...
select pno, pname, dno from person 
where dno=10 or dno=20;

-- 4. 보너스가 null인 사원의 사번, 이름, 급여, 보너스. 급여 큰 순정렬
select pno, pname, sal, comm from person 
where comm is null order by sal desc;

-- 5. 사번, 이름, 부서번호, 급여. 부서코드 순 정렬 같으면 PAY 큰순
select pno, pname, dno, sal from person 
order by dno asc, sal desc;

-- 6. 사번, 이름, 부서명
select p.pno, p.pname, d.dname 
from person p, division d where p.dno = d.dno;
...
from person p1, person p2 where p1.manager = p2.pno;

select p1.pno, p1.pname, p2.panme
...
-- 8.2 사번, 이름, 급여, 고액연봉자(4000이상) or 일반연봉가
...
select p1.pno, p1.pname, p1.sal, d.dname, p2.pname
from person p1, division d, person p2
where p1.dno = d.dno && p1.manager = p2.pno;

select p1.pno, p1.pname, p1.sal, d.dname, p2.pname
from person p1 join division d on p1.dno=d.dno
			   join person p2 on p1.manager = p2.pno;
               
select p1.pno, p1.pname, p1.sal, d.dname, p2.pname
from person p1 join division d on p1.dno=d.dno
			   left join person p2 on p1.manager = p2.pno; -- 상사가 없는 사람도 출력
