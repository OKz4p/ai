...
SELECT * FROM TAB; -- 현계정이 갖고 있는 테이블 정보
SELECT * FROM EMP; -- EMP테이블의 모든 정보(모든 열, 모든 행)
...
-- 4. 특정 행만 출력 : WHERE절(조건절)에서 논리연산자 : OR, AND, NOT
    -- EX1. 급여(SAL)가 2000부터 3000인 직원의 모든 필드 출력
    SELECT * FROM EMP WHERE 2000<=SAL AND SAL<=3000;
    -- EX2. 82년도에 입사한 사원의 모든 필드
    SELECT * FROM EMP WHERE HIREDATE>='82/01/01' AND HIREDATE<='82/12/3';
    SELECT * FROM EMP
...
-- 5. 산술연산자(SELECT절, WHERE절, ORDER BY절)
    -- EX. 연봉이 2400이상인 직원의ENAME, SAL, 연봉(SAL*12)
    ...
    -- EX. 연봉이 10000이상인 직원의 ENAME, SAL, 연봉(SAL*12+COMM
        ...
        -- NVL(NULL일수도 있는 필드명, 대체값) - 필드명과 대체값의 타입이 일치
...
SELECT ENAME || '은' || JOB FROM EMP;
    -- EX. 모든 사원에 대해서 "SMITH의 연봉 XXXX$입니다"라고 출력하고 TITLE에는 MESSAGE.
...
SELECT EMPNO 사번, ENAME 이름, SAL 급여, HIREDATE 입사일, JOB 업무 FROM EMP;

--5. emp 테이블에서 급여가 2000미만인 사람의 사번, 이름, 급여 출력
SELECT EMPNO NO, ENAME NAME, SAL FROM EMP WHERE SAL<2000;

--6. 입사일이 81/02이후에 입사한 사람의 사번, 이름, 업무, 입사일 출력
SELECT EMPNO 사번, ENAME 이름, JOB 업무, HIREDATE 입사일 FROM EMP WHERE HIREDATE >= TO_DATE('81/03/01');

--7. 업무가 SALESMAN인 사람들 모든 자료 출력
SELECT * FROM EMP WHERE JOB='SALESMAN';

--8. 급여가 1500이상이고 3000이하인 사람의 사번, 이름, 급여 출력
SELECT EMPNO 사번, ENAME 이름, SAL 급여 FROM EMP WHERE 1500<=SAL AND SAL<=3000;
...
-- 8. SQL 연산자(BETWEEN, IN, LIKE, IS null)
 -- (1) 필드명 BETWEEN A AND B : 필드명이 A부터 B까지(A, B포함) A<B
    --  필드명 NOT BETWEEN A AND B : 필드명이 A부터 B까지(A, B포함) A<B
    ...
    SELECT * FROM WHERE SAL NOT BETWEEN 1500 AND 3000;

 -- (2) 필드명 IN (값1, ... 값N) : 필드명이 값1이거나 값2이거나, ... 값N인 경우
...
-- (4) IS NULL(NULL인지를 검색할때)
    ...
    SELECT * FROM EMP WHERE COMM IN (NULL, 0); -- 불가
    SELECT * FROM EMP WHERE COMM=NULL OR COMM=0; -- 불가
    SELECT * FROM EMP WHERE COMM IS NULL OR COMM=0 ;
    -- EX. 상여금을 받는 사원의 모든 필드(COMM이 CULL이 아니고 COMM이 0이 아닌 사원)
...
SELECT EMPNO, ENAME, SAL FROM EMP WHERE (SAL*12+COMM)>=2400 ORDER BY (SAL*12+COMM)DESC;
...
--      급여가 같으면 입사일이 빠른 사원으로 정렬하시오. 급여 내림차순, 같으면 입사일 내림차순
SELECT EMPNO ENAME, HIREDATE, JOB, SAL FROM EMP ORDER BY SAL DESC;
