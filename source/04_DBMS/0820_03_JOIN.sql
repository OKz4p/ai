...
SELECT * FROM EMP, DEPT WHERE ENAME='SCOTT'; --EMP의 1행 * DERP의 4행
...
SELECT E.*, DNAME, LOC 
  ...
  -- EX. 근무지(LOC)가 'CHICAGO' 인 직원의 이름, 직책, 급여, 부서번호
...
--  EX. 직급이 MANAGER인 사원의 이름, 부서명을 출력하시오
    SELECT ENAME, HIREDATE 
    FROM EMP E, DEPT D 
    WHERE E.DEPTNO=D.DEPTNO 
    AND JOB='MANAGER';

--	EX. Comm이 null이 아닌 사원의 이름, 급여, 부서코드, 근무지를 출력하시오.
    SELECT ENAME, SAL, E.DEPTNO, LOC 
    FROM EMP E, DEPT D 
    WHERE E.DEPTNO=D.DEPTNO
    AND COMM IS NOT NULL;
    
-- EX. 20번부서외의 사원의 이름, 급여, 부서코드, 근무지를 부서코드순으로 출력하시오
    SELECT ENAME, HIREDATE 
    FROM EMP E, DEPT D 
    WHERE E.DEPTNO=D.DEPTNO
    AND E.DEPTNO!=20
...
-- 연습 예제    
-- EX.	Comm이 null이 아닌 사원의 이름, 급여, 등급, 부서번호, 부서이름, 근무지를 출력하시오.
...
-- EX.	이름, 급여, 급여등급, 연봉, 부서명을 부서명순으로 정렬하여 출력. 부서가 같으면 연봉순. 연봉=(sal+comm)*12 comm이 null이면 0
  SELECT ENAME, SAL, GRADE, (SAL+NVL(COMM, 0))*12 ANNUAL_SAL, DNAME
    FROM EMP E, DEPT D, SALGRADE
    WHERE E.DEPTNO=D.DEPTNO AND SAL BETWEEN LOSAL AND HISAL
    ORDER BY DNAME, (SAL+NVL(COMM, 0))*12

-- EX.	이름, 업무, 급여, 등급, 부서코드, 부서명 출력. 급여가 1000~3000사이. 정렬조건 : 부서별, 부서같으면 업무별, 업무같으면 급여 큰순
...
-- EX.	이름, 급여, 등급, 입사일, 근무지. 81년에 입사한 사람. 등급 큰순
...
SELECT EMPNO, ENAME, MGR FROM EMP WHERE ENAME='SMITH'; -- 1행(WORKER)
SELECT EMPNO, ENAME FROM EMP; -- 14행(MANAGER)
...
-- (1) SELF-JOIN에서의 OUTTER JOIN : 조건에 배제된 행을 결과에 포함시킬 경우, +기호를 정보가 부족한 컬럼에 붙이기
...
SELECT * FROM EMP E, DEPT D
    WHERE E.DEPTNO=D.DEPTNO; --40번 부서 출력 안 됨
SELECT * FROM EMP E, DEPT D
    WHERE E.DEPTNO(+)=D.DEPTNO; --40번 부서 출력됨    
...
SELECT EMPNO, ENAME, E.JOB, E.SAL, S.GRADE, DNAME FROM EMP E, DEPT D, SALGRADE S
WHERE E.DEPTNO=D.DEPTNO 
AND E.SAL BETWEEN S.LOSAL AND S.HISAL
AND E.JOB='MANAGER' 
AND E.SAL>=2500 ORDER BY EMPNO ASC;
...
SELECT W.ENAME, W.SAL, D.DEPTNO, D.DNAME, D.LOC, NVL(M.ENAME, '없음')
    FROM EMP W, EMP M, DEPT D
    WHERE W.MGR=M.EMPNO(+)
    AND W.DEPTNO=D.DEPTNO;
...
SELECT W.ENAME, W.SAL, S.GRADE, D.DNAME, (W.SAL + NVL(W.COMM, 0))*12, M.ENAME
    FROM EMP W, EMP M, DEPT D, SALGRADE S
    WHERE W.MGR=M.EMPNO(+)
    AND W.DEPTNO=D.DEPTNO
    AND W.SAL BETWEEN S.LOSAL AND S.HISAL;
